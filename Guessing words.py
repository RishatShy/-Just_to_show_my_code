...
def random_word():
    word_list = ['человек', 'работа', 'вопрос', 'сторона', 'страна', 'случай', 'голова', 'ребенок', 'система',
                 ...
                 'количество', 'событие', 'объект', 'создание', 'значение', 'период', 'искусство', 'структура',
                 'пример',
                 ...
                 'музыка', 'правда', 'свобода', 'память', 'команда', 'договор', 'дерево', 'хозяин', 'природа',
                 'телефон',
                 'позиция', 'писатель', 'самолет', 'солнце', 'спектакль', 'способ', 'журнал', 'руководитель',
                 'специалист',
                 'оценка', 'регион', 'процент', 'родитель', 'требование', 'основание', 'половина', 'анализ',
                 'автомобиль',
                 ...
                 'магазин', 'пространство', 'знание', 'защита', 'руководство', 'площадь', 'сознание', 'возраст',
                 'участник',
                 ...
                 'командир', 'коридор', 'поддержка', 'собрание', 'болезнь', 'клетка', 'заявление', 'попытка',
                 'сравнение',
                 ...
                 'передача', 'способность', 'подход', 'энергия', 'существование', 'исполнение', 'сожаление',
                 'заместитель',
                 ...
                 'родина', 'собственность', 'лагерь', 'имущество', 'кровать', 'аппарат', 'середина', 'клиент',
                 'отрасль',
                 ...
                 'образец', 'главное', 'костюм', 'ценность', 'обязанность', 'таблица', 'воспоминание', 'лошадь',
                 'коллега',
                 'организм', 'ученик', 'учреждение', 'открытие', 'характеристика', 'выполнение', 'оборона',
                 'выступление',
                 ...
                 'присутствие', 'взаимодействие', 'партнер', 'двигатель', 'достоинство', 'страсть', 'испытание',
                 'оплата',
                 'разница', 'водитель', 'снижение', 'формула', 'капитал', 'новость', 'эффект', 'губернатор', 'доклад',
                 'убийство', 'эксперт', 'автобус', 'платье', 'общение', 'психология', 'проверка', 'процедура',
                 'рабочий',
                 ...
                 'эксперимент', 'печать', 'кольцо', 'пистолет', 'воспитание', 'начальство', 'профессия', 'ворота',
                 'дружба',
                 ...
                 'чтение', 'поселок', 'свидетель', 'ставка', 'удивление', 'поворот', 'возвращение', 'мгновение',
                 'статус',
                 'параметр', 'сказка', 'тенденция', 'дыхание', 'версия', 'масштаб', 'монастырь', 'хозяйка',
                 'эксплуатация',
                 'коммунист', 'пенсия', 'приятель', 'объяснение', 'производитель', 'философия', 'мощность',
                 'обязательство',
...
def make_unscopes(word):
    coded_word = []
    for c in word:
        coded_word.append(' ___ ')
    return coded_word


def correct_input(used_letters, coded_word):
    letter = input('Введите предполагаемую букву:\n').upper()

    if letter in used_letters:
        print('Эта буква уже была. Как и эти буквы:')
        print(*used_letters)
        print('Введите другую')
        return correct_input(used_letters, coded_word)

    if (len(letter) == 1) and (1039 < ord(letter) < 1072):
        return letter
    else:
        print('Этот символ является недопутимым. Напомню, что используются буквы русского алфавита')
        return correct_input(used_letters, coded_word)


def if_win(word, coded_word):
    print(*coded_word, sep='')
    if input('У вас есть предположения?(да, нет)\n').lower() == 'да':
        if input('Как вы думаете, что это за слово?\n').upper() == word:
            print('Ура!!! Вы выиграли. Казнь отменяется')
            return True
        else:
            print('Не верно')
            time.sleep(3)
        ...
        print('Ничего страшного, потом угадаете')
        time.sleep(3)
        return False


def fill_unscopes(letter, coded_word, word):
    modified_word = word

    while letter in modified_word:
        coded_word[modified_word.index(letter)] = letter
...
def play():
    ...
    while attempt > 0:
        print(word)
        print('\n\n\n\n\n\n\n\n\n\n\n', display(attempt))
        print(f'Вы можете сделать предположение еще столько раз: {attempt}')
        print(*coded_word, sep='')
        letter = correct_input(used_letters, coded_word)
        used_letters += letter
        fill_unscopes(letter, coded_word, word)
        attempt -= 1
        if if_win(word, coded_word):
            print(display(7))
            break
    else:
        print('\n\n\n\n\n', display(0))
...
def greetings():
    ...
    if input('Введите "ПРАВИЛА", если хотите с ними ознакомиться. Если нет - любой другой знак\n').lower() == 'правила':
        print('''
                Я загадываю слово. Согласно традиции русских лингвистических игр, слово должно быть именем 
              существительным, нарицательным в именительном падеже единственного числа, либо множественного 
              числа при отсутствии у слова формы единственного числа. Ваша задача его угадать, у вас будет 6 попыток, 
              если вы не уложитесь игра считается проигранной, а вы повешенным. С каждой использованной попыткой я 
              буду добавлять по части виселицы
              ''')
    input('Если вы ознакомились с правилами введите любой символ\n')
...
while True:
    play()
    if input('Хотите сыграть еще?(да, нет)') != 'да':
        break
